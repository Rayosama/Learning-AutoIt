...
global $huntingForColor = 0xD5D30D ;0x057508 Or  Or 0x057508
...
;	$pix2 = PixelSearch(970, 323, 970, 323, 0xFCFCFC, 1)
   $pix2 = PixelSearch(260, 144, 260, 144, 0x52410D, 5)
   if Not(@error) Then
	  VoldemortTele()
   EndIf
   ...
   ;find a mob
   $pix = PixelSearch($x1, $y1, $x2, $y2, $huntingForColor, 2) ; First two number sets are coords on the screen for our bounds.Then color code, and then shade difference
...
Func VoldemortTele()
   ...
   MouseClick("left", 619, 431, 2, 1) ;clicks Clicks O.G Dragon from the options
	  Sleep(2500)

    MouseClick("left", 777, 530, 2, 1) ;clicks Clicks O.G Dragon from the options
	  Sleep(2500)
