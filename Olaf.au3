...
global $huntingForColor = 0x294846    ; 0xCF8951
...
while(1)
   ...
   $death = PixelSearch( 1660, 95,1660, 95, 0x890302, 5)
   if @error Then
	  Sleep(8000)
	  OlafTele()
	  Sleep(1000)
   ...
   if (not(@error) And $Hunting) Then ;This means we found it and Hunting is True
	  ...
	  MouseClick("left", $pix[0], $pix[1],2,2) ;Left click using the array at index x=0, y=5, click once, per 1 sec
	  sleep(2200)
	  $attacking = true;

	  while($attacking) ;while the variable attacking is true, this while loop will run
	  ;are we still attacking the mod?
	  	  $isAttacking = PixelSearch(10, 79, 10, 79, 0x105B10, 5) ;The 1 for color variable at the end means it's looking for an exact match
...
Func OlafTele()
   MouseClick("left", 1652, 1060, 2, 2)
   Sleep(500)
   MouseClick("left", 1756, 843, 2, 2)
   Sleep(500)
   MouseClick("left", 612, 395, 2, 2)
   Sleep(500)
   MouseClick("left", 786, 531, 2, 2)
   Sleep(500)
   MouseClick("left", 1835, 764, 2, 2)
   Sleep(500)
   MouseClick("left", 1682, 844, 1, 1)
   Sleep(500)
   MouseClick("left", 1775, 766, 1, 1)
EndFunc

Func Rotate()
   Send("{left down}") ;Holds the left arrow key down
   Sleep(2000)
