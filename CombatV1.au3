...
global $huntingForColor = 0xBBBBB1    ; 0xCF8951
...
while(1)
   ...
   if (not(@error) And $Hunting) Then ;This means we found it and Hunting is True
	  ...
	  MouseClick("right", $pix[0], $pix[1],1,1) ;Left click using the array at index x=0, y=5, click once, per 1 sec
	  sleep(1000)

	  $pix2 = PixelSearch($x1, $y1, $x2, $y2, 0xFFFF00, 1) ; First two number sets are coords on the screen for our bounds.Then color code, and then shade difference
	  if Not(@error) Then
		 MouseClick("left", $pix2[0], $pix2[1], 2, 1)
	  EndIf
