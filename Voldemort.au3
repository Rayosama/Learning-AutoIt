...
while(1)
   ...
   if (not(@error) And $Hunting) Then ;This means we found it and Hunting is True
	  ...
	  MouseClick("left", $pix[0], $pix[1],2,1) ;Left click using the array at index x=0, y=5, click once, per 1 sec
	  sleep(8000)
	  $attacking = true;
...
Func VoldemortTele()
   ...
   MouseClick("left", 781, 529, 2, 1) ;clicks voldemort from the options
	  Sleep(2500)
